...
\begin{frame}{Introduction}
  ...
  \item \Neromum{} -- a \Nerokid{} manager daemon that helps the family stay
    organized while the kids are playing in distant clusters.
    \Neromum{} is designed to collaborate with a \texttt{warden} (f. ex.
    SLURM, OGE or Jobman) if necessary
...
\begin{frame}{Introduction}
  ...
  \item \Neromum{} is a job scheduler, manager and communications agent
    for Linux nodes or clusters designed to work by herself or with an
    existing \texttt{warden} (job scheduler)
...
\begin{frame}{Components}
  ...
  \item \Neromum{} communicates with any \texttt{warden}s using their CLIs
    and/or APIs
...
\begin{frame}{Communication}
  ...
  \resizebox{0.8\columnwidth}{!}{
    \begin{tikzpicture}
      ...
      \node (mm) at ( 6, 5.5) [nmum,fill=yellow!30]  {\texttt{neromum}};
...
\begin{frame}{\texttt{neroman}}
  ...
  \item Key functionality
    ...
    \item Batch submit experiment jobs to a \texttt{warden} with autogenerated
...
\begin{frame}{\texttt{neromum}}
  ...
  \item Ment to work with \texttt{warden}s -- standard job scheduling systems
    for Linux clusters such as SLURM, OGE or Jobman.
    \begin{itemize}
  ...
  \item Key functionality
    \begin{itemize}
    \item Collect information from \Nerokid{}s and send them to \Neroman{}
