...
class Experiment(object):
    """An object to represent experiments as used by Neronet

    Experiment objects are generated by neronet.config_parser, based on 
    data read from a config.yaml. The idea behind this is that every
    experiment has its own folder, and all its variations (or at least those
    achieved by varying the experiment's parameters) are then defined in 
    the config.yaml.
    ...
    class State:
        """All the possible states of an Experiment"""
        #none = 'none'
        defined = 'defined'
        submitted = 'submitted'
    ...
    def get_results_dir(self):
        """Builds the path to local experiment results folder

        Returns:
            str: the path to ~/.neronet/results/experiment_id 
                or if the experiment is finished, the path to the 
                original experiment folder + /results/experiment_id.
        """
